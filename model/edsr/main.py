...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--upscale", help="Upscale an image with desired scale", action="store_true"
    )
    parser.add_argument("--export", help="Export the model as .pb", action="store_true")
    parser.add_argument(
        "--fromscratch", help="Load previous model for training", action="store_false"
    )
    ...
    parser.add_argument(
        "--scale", type=int, help="Scaling factor of the model", default=2
    )
    parser.add_argument(
        "--batch", type=int, help="Batch size of the training", default=16
    )
    parser.add_argument(
        "--epochs", type=int, help="Number of epochs during training", default=20
    )
    parser.add_argument("--lr", type=float, help="Learning_rate", default=0.0001)

    # paths
    parser.add_argument(
        "--image", help="Specify test image", default="./images/original.png"
    )
