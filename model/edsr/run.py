...
class run:
    def __init__(
        self, config, ckpt_path, scale, batch, epochs, B, F, lr, load_flag, meanBGR
    ):
        self.config = config
    ...
    def train(self, imagefolder, validfolder):
        ...
        train_dataset = train_dataset.padded_batch(
            self.batch, padded_shapes=([None, None, 3], [None, None, 3])
        )
        ...
        val_dataset = val_dataset.padded_batch(
            1, padded_shapes=([None, None, 3], [None, None, 3])
        )

        # Make the iterator and its initializers
        train_val_iterator = tf.data.Iterator.from_structure(
        ...
        with tf.Session(config=self.config) as sess:
            ...
            for e in range(1, self.epochs + 1):
                ...
                try:
                    while True:
                        o, l, t, l_rate = sess.run(
                            [out, loss, train_op, lr],
                            feed_dict={
                                handle: train_val_handle,
                        ...
                        if step % 1000 == 0:
                            ...
                            print(
                                "Step nr: [{}/{}] - Loss: {:.5f} - Lr: {:.7f}".format(
                                    step, "?", float(train_loss / step), l_rate
                                )
                            )
                ...
                try:
                    while True:
                        val_psnr, val_ssim = sess.run(
                            [psnr, ssim], feed_dict={handle: train_val_handle}
                        )
                ...
                print(
                    "Epoch nr: [{}/{}]  - Loss: {:.5f} - val PSNR: {:.3f} - val SSIM: {:.3f}\n".format(
                        e,
                        self.epochs,
    ...
    def test(self, path):
        ...
        with tf.Session(config=self.config) as sess:
            ...
            print(
                "PSNR of  EDSR   upscaled image: {}".format(
                    self.psnr(cropped, HR_image)
                )
            )
            print(
                "PSNR of bicubic upscaled image: {}".format(
                    self.psnr(cropped, bicubic_image)
                )
            )
        ...
        with graph.as_default():
            with tf.Session(config=self.config) as sess:
                ...
                graph_def = TransformGraph(
                    graph_def, ["IteratorGetNext"], ["NCHW_output"], transforms
                )
