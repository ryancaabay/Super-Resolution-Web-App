...
class VGG(Module):
    ...
    def forward(self, inputs):
        ...
        inputs = inputs * 255.0 - tlx.convert_to_tensor(
            np.array([123.68, 116.779, 103.939], dtype=np.float32).reshape(-1, 1, 1)
        )
    ...
    for layer_group_idx, layer_group in enumerate(config):
        if isinstance(layer_group, list):
            for idx, layer in enumerate(layer_group):
                ...
                if batch_norm:
                    layer_list.append(
                        BatchNorm(num_features=n_filter, data_format="channels_first")
                    )
            ...
            if layer_group == "M":
                ...
                layer_list.append(
                    Linear(out_features=1000, in_features=4096, name=layer_name)
                )
            elif layer_group == "F":
...
def restore_model(model, layer_type):
    ...
    maybe_download_and_extract(
        model_saved_name[layer_type], "model", model_urls[layer_type]
    )
    weights = []
    if layer_type == "vgg16":
        npz = np.load(
            os.path.join("model", model_saved_name[layer_type]), allow_pickle=True
        )
        # get weight list
        ...
        npz = np.load(
            os.path.join("model", model_saved_name[layer_type]),
            allow_pickle=True,
            encoding="latin1",
        ).item()
        # get weight list
